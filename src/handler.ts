 ...
 */
const commandsEnabled = runtimeCfg?.commands?.enabled ?? {
  ping: true,
  marco: true,
  roll: true,
  logout: true,
...
export function getReplyForText(text: string): Reply | null {
  ...
  if (/^!marco\s*$/i.test(lower))
    return commandsEnabled?.marco ? { text: 'polo... or was it Paulo? 🧲🎤' } : null;

  // help command: build a message listing enabled commands
  if (/^!help\s*$/i.test(lower)) {
    const enabled = Object.entries(commandsEnabled)
      .filter(([_, v]) => v)
      .map(([k]) => k);
  ...
  if (/^!shutdown\s*$/i.test(lower))
    return commandsEnabled?.shutdown ? { text: '!shutdown' } : null;
  ...
  if (parsed !== null) {
    ...
    const textRes =
      results.length === 1
        ? `🎲 You rolled a *${results[0]}* on the d${sides}!`
        : `🎲 You rolled: ${results.join(', ')} (d${sides})`;
    return { text: textRes };
  }
