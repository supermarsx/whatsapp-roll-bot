...
export function startLogRotationMonitor(
  ...
  async function sendWebhook(payload: any) {
    ...
    try {
      ...
      enqueueLog('warn', 'Log rotation monitor: failed to call webhook: ' + ((e && (e as Error).message) || e));
    }
  }
    ...
    try {
      ...
      for (const f of files) {
        ...
        try {
          ...
          enqueueLog('warn', `Log rotation monitor: failed to purge ${f}: ${(e && (e as Error).message) || e}`);
        }
      }
      return removed;
    } catch (e) {
      enqueueLog('warn', 'Log rotation monitor: purge failed: ' + ((e && (e as Error).message) || e));
      return 0;
    }
  }
    ...
    try {
      ...
      for (const fname of files.filter(f => f.startsWith('application-')).slice(-30)) {
        try {
          ...
          if (st.size > maxSize) {
            enqueueLog('warn', `Log rotation monitor: file ${fname} is large (${Math.round(st.size / 1024 / 1024)} MB)`);
            suspect.push(fname);
          }
        } catch (e) {
          enqueueLog('warn', `Log rotation monitor: failed to stat ${fname}: ${(e && (e as Error).message) || e}`);
        }
      }
  ...
  return {
    ...
    purgeOldFiles: (d?: string) => purgeOldFiles(path.resolve(process.cwd(), d || logsDir)),
      ...
      if (!hasToday) {
        enqueueLog('warn', `Log rotation monitor: today's log not found: ${todayName}`);
      }
      if (!hasYesterday) {
        enqueueLog(
          'info',
          `Log rotation monitor: no rotated file found for yesterday (${yesterName}) — this may be fine on first run`,
        );
      }

      // check sizes of recent files
      for (const fname of files.filter(f => f.startsWith('application-')).slice(-10)) {
        try {
          const st = await fs.stat(path.join(dir, fname));
          if (!st.isFile()) continue;
          if (st.size === 0) {
