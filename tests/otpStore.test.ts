...
describe('EncryptedOtpStore', () => {
  ...
  afterAll(async () => {
    ...
    const store2 = new EncryptedOtpStore({ dataDir: tmpDir, keyHex: key, otpTTL: 60, otpLength: 4 })
  ...
  test('cleanup removes expired entries', async () => {
    const shortStore = new EncryptedOtpStore({ dataDir: tmpDir, keyHex: key, otpTTL: 1, otpLength: 3 })
