...
describe('access control', () => {
  ...
  test('group whitelist works', () => {
    ...
    expect(ac.isMessageAllowed({ text: '!ping', from: 'someone@c.us', isGroup: true, groupJid: '123@g.us' })).toBe(true)
    expect(ac.isMessageAllowed({ text: '!ping', from: 'someone@c.us', isGroup: true, groupJid: '999@g.us' })).toBe(false)
  })
  ...
  test('admin command allowed only for admin and whitelisted', () => {
    const ac = createAccessControl({ admin: { admins: ['admin@c.us'], commands: ['!shutdown'], whitelist: { contacts: ['admin@c.us'] } } })
