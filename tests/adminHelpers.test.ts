...
describe('admin helpers', () => {
  ...
  test('setAdminChannel writes config and returns true', async () => {
    ...
    expect(config.writeConfig).toHaveBeenCalledWith('config.json', expect.objectContaining({ admin: expect.any(Object) }))
  })
  ...
  test('unsetAdminChannel removes adminChannel and returns true', async () => {
    ...
    expect(ok).toBe(true)
    expect(config.readConfig).toHaveBeenCalledWith('config.json')
    expect(config.writeConfig).toHaveBeenCalledWith('config.json', expect.not.objectContaining({ admin: { adminChannel: '123@g.us' } }))
  })
})
